...
/* Huge thanks to https://github.com/HenriDellal/kernel-neffos-c7s
...
static void sprd_unplug_cpus(struct work_struct *work)
{
	...
	if (num_online_cpus() > sd_tuners->cpu_num_limit) {
		...
		for_each_online_cpu(cpu) {
			if (0 == cpu)
				continue;
			pr_debug("!!  all gonna unplug cpu%d  !!\n", cpu);
			cpu_down(cpu);
			if (--be_offline_num <= 0)
				break;
		}
...
static void sprd_plugin_cpus(struct work_struct *work)
{
	...
	if (num_online_cpus() < max_num) {
		...
		for_each_possible_cpu(cpu) {
			if (!cpu_online(cpu)) {
				pr_debug("!! all gonna plugin cpu%d  !!\n",
						cpu);
				cpu_up(cpu);
				if (--be_online_num <= 0)
					break;
...
static unsigned int sd_unplug_avg_load1(int cpu, struct sd_dbs_tuners *sd_tunners , unsigned int load)
{
	...
	*/
	...
	if(avg_load > 100)
	{
		pr_debug("cur_window_pos %d cur_window_pos_tail %d load %d sum_load %d\n",cur_window_pos,cur_window_pos_tail,load,sum_load[cpu] );
	}
	return avg_load;
...
static void sd_check_cpu(int cpu, unsigned int load)
{
	...
	if (true == sd_tuners->is_suspend) {
		pr_debug("%s: is_suspend=%s, skip cpufreq adjust\n",
	...
	unsigned long event, void *dummy)
{
	...
	if (NULL == dbs_data) {
		pr_debug("sprdemand_gov_pm_notifier_call governor %s return\n", policy->governor->name);
		if (g_sd_tuners == NULL)
			return NOTIFY_OK;
		sd_tuners = g_sd_tuners;
	} else {
	...
	if (event == PM_SUSPEND_PREPARE || event == PM_HIBERNATION_PREPARE) {
		pr_debug(" %s, recv pm suspend notify\n", __func__ );
		...
		g_is_suspend = true;
		pr_debug(" %s, recv pm suspend notify done\n", __func__ );
	}
